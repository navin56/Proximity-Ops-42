...
void Report(void)
{
      ...
      if (First) {
         ...
         /* Add File for logging SC1 States. Pos, Vel. */
         PosNfile1 = FileOpen(InOutPath,"PosN1.42","w");
         VelNfile = FileOpen(InOutPath,"VelN.42","w");
         VelNfile1 = FileOpen(InOutPath,"VelN1.42","w");
      ...
      if (OutFlag) {
         ...
         if (SC[0].Exists) {
            ...
            /* Log SC[1] States for Debug. */
            if (SC[1].Exists)
            {
               fprintf(PosNfile1,"%le %le %le\n",
                  SC[1].PosN[0],SC[1].PosN[1],SC[1].PosN[2]);
               fprintf(VelNfile1,"%le %le %le\n",
